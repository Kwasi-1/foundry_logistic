...
import { DateSelector, DateSelectorValue } from "./date-selector"
...
const singleDateRanges: Record<string, RelativeRange> = {
  today: {
    ...
    getValue: () => ({ mode: "single", date: new Date().toISOString().slice(0, 10) }),
  },
  yesterday: {
    label: "Yesterday",
...
export function DateFilter({
  ...
  const [customValue, setCustomValue] = React.useState<DateSelectorValue>(initialCustomValue)
  const relativeRanges = selectionMode === "single" ? singleDateRanges : dateRangeRanges
  ...
  return (
    <div className={cn("w-full", className)}>
      <Tabs value={mode} onValueChange={(value) => setMode(value as DateFilterMode)}>
            ...
            {Object.entries(relativeRanges).map(([key, preset]) => (
              <button
                ...
                className={cn(
                  ...
                  relativeKey === key ? "bg-primary/10 text-primary border border-primary" : "hover:bg-muted/60 border border-transparent",
                )}
              >
